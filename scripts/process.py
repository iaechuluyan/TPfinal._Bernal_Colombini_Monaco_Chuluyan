...
class process: 
    ...
    Methods:
    --------
    decide_function (self, a_time, a_end, s_time, s_tot_time, d_time, d_tot_time): uses a dictionary to interpret 
    which attack, sustain and decay function to use depending on the ones specified on the harmonics text file.
    ...
    set (self, s_audio): adds the note to the main numpy array (s_audio) at the correct position.
    ...
    def decide_function(self, a_time, a_end, s_time, s_tot_time, d_time, d_tot_time):  #aca ta el error
        ...
        if self.modulation_info[0][0] in ['TRI']:
            a = self.dict[self.modulation_info[0][0]](a_time, a_end, float(self.modulation_info[0][2]), float(self.modulation_info[0][3]), self.frate)
        else:
        ...
        if self.modulation_info[1][0] in ['CONSTANT']:
            ...
            s = self.dict[self.modulation_info[1][0]](s_time, s_tot_time, self.modulation_info[1][2], self.modulation_info[1][3])
        else:
